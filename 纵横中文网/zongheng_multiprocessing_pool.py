#!/usr/bin/env python
# -*- encoding: utf-8 -*-
# @File    :   zongheng_multiprocessing_pool.py
# @Time    :   2020/4/23 16:49
# @Author  :   LJL
# @Version :   1.0
...
class ZongHeng(object):
    def __init__(self, page):
        ...
        self.queue_detail = Queue()
        self.pool_detail = Pool(processes=3)
    ...
    def main(self):
        ...
        while not self.queue.empty():
            url = self.queue.get()
            self.pool.apply_async(self.geturl(url))
        ...
        while not self.queue_detail.empty():
            contents = self.queue_detail.get()
            self.pool_detail.apply_async(self.getdetail(contents))
            time.sleep(random.uniform(0, 2))

        self.pool_detail.close()
        self.pool_detail.join()
