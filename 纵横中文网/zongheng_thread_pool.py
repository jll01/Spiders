...
class ZongHeng(object):
    def __init__(self, page):
        ...
        self.pool_detail = ThreadPoolExecutor(3)
    ...
    def main(self):
        ...
        time.sleep(1)
        # 当队列不为空的时候，在进线程池中进行任务
        while not self.queue.empty():
            url = self.queue.get()
            self.pool.submit(self.geturl(url))
        self.pool.shutdown()
        ...
        while not self.queue_detail.empty():
            ...
            self.pool_detail.submit(self.getdetail(contents))
        self.pool_detail.shutdown()
