...
class ZongHeng(object):
    ...
    def main(self):
        ...
        while not self.queue.empty():
            url = self.queue.get()
            self.pool.add(gevent.spawn(self.geturl, url))
        ...
        #     self.pool_detail.add(gevent.spawn(self.getdetail, contents))
        ...
        while not self.queue_detail.empty():
            contents = self.queue_detail.get()
            g_list.append(gevent.spawn(self.getdetail, contents))
            time.sleep(random.uniform(0, 2))
