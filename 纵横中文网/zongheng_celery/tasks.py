...
from zongheng_celery.app import app
...
def parser(url):
    ...
    if request.status_code == 200:
        ...
        for url, title, author, author_url, category, category_url, status, update_time, intro, book_update, update_url in all_contents:
            ...
            item['status'] = status_strip(status)
            item['update_time'] = update_time_re(update_time)
            item['intro'] = intro
...
def getdetail(contents):
    """获取每本书的详细信息"""
    detail_con = requests.get(contents['url'], headers={'User-Agent': UserAgent().random}).text
    ...
    contents['number'] = join_str(number)
    contents['all_recommend'] = join_str(all_recommend)
    contents['click_num'] = join_str(click_num)
    contents['week_recommend'] = join_str(week_recommend)
    print(contents)
    # save_json(contents)
    save_mysql(contents)
