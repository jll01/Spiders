#!/usr/bin/env python
# -*- encoding: utf-8 -*-
# @File    :   zongheng_asyn.py
# @Time    :   2020/4/28 18:32
# @Author  :   LJL
# @Version :   1.0
    ...
    async def geturl(self, url):
        ...
        async with self.sem:
            async with aiohttp.ClientSession() as session:
                async with await session.get(url, headers=header) as req:
                    res = await req.read()
                    html = etree.HTML(res)
                    # 书的链接q
                    ...
                    for url, title, author, author_url, category, category_url, status, update_time, intro, book_update, update_url in all_contents:
                        ...
                        await self.getdetail(item)

    async def getdetail(self, contents):
        """获取每本书的详细信息"""
        async with self.sem:
            async with aiohttp.ClientSession() as session1:
                async with await session1.get(contents['url'], headers={'User-Agent': self.useragent.random}) as req:
                    res = await req.read()
                    # detail_con = requests.get(contents['url'], headers={'User-Agent': self.useragent.random}).text
                    html_con = etree.HTML(res)
                    # 字数
                    ...
                    await self.save_mysql(contents)
